...
const numbersImpar = randomNumbers.filter(number => number%2===1)
...
const number501 = crazyNumbers.reduce((accumulator, crazynumber) => {
  if(crazynumber<501){
    return accumulator += 1
  }
  return accumulator // precisa colocar em tudo, pois se usa quando a função tem que retornar um valor
},0) // tem que colocar o zero, para setar o 1 valor do accumulator em zero

/* console.log(number501) */
...
const numberDouble = numbers.map(number => Math.pow(number,2))
...
const movies2000 = tarantinoMovies.filter(tarantinoMovie => tarantinoMovie.release<2000)
...
const nomeSeries = tvShows.map(tvShow => tvShow.name)
...
const jogos = cart.reduce((accumulator, jogo)=>{

  return (`${accumulator} - ${jogo.name}\n`)
}, "")

/* const jogos = cart.reduce((accumulator, { name})=> (`${accumulator} - ${name}\n`), "") */
